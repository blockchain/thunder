...
import network.thunder.client.database.objects.*;
import network.thunder.client.database.objects.KeyWrapper.IKey;
import network.thunder.client.etc.*;
...
public class MySQLConnection {
	 ...
	 */
	public static Transaction getOutAndInputsForChannel (Connection conn, ArrayList<Output> list, int channelId, long myShare, Transaction transactionOld,
	                                                     Address changeAddress, boolean sign, boolean withFees) throws SQLException, AddressFormatException {
		PreparedStatement stmt = null;
		ResultSet set = null;
		try {
	...
	public static ArrayList<Payment> getPaymentsIncludedInChannelWithNoSecret (Connection conn, int channelId) throws SQLException {
		...
		try {
			...
			stmt = conn.prepareStatement("SELECT * FROM payments WHERE ( ( (channel_id_sender=? AND include_in_sender_channel=1 AND phase!=6) OR " + "" +
					"(channel_id_receiver=? AND include_in_receiver_channel=1 AND phase!=5 AND phase!=6) ) AND secret IS NULL) ORDER BY id ASC");
			stmt.setInt(1, channelId);
			stmt.setInt(2, channelId);
			ResultSet results = stmt.executeQuery();
	...
	public static ArrayList<Payment> getPaymentsIncludedInChannelWithPaymentsNotAddedYet (Connection conn, int channelId, int amount) throws SQLException {
		...
		try {
			...
			stmt = conn.prepareStatement("SELECT * FROM payments WHERE ( (channel_id_receiver=? AND include_in_receiver_channel=0) AND phase != 6 AND secret "
	...
	public static ArrayList<Payment> getPaymentsOpen (Connection conn, int channelId) throws SQLException {
		...
		try {
			ArrayList<Payment> list = new ArrayList<Payment>();
			stmt = conn.prepareStatement("SELECT * FROM payments WHERE ( channel_id_receiver=? AND include_in_receiver_channel=0 AND phase=0 ) ORDER BY id " +
					"DESC");
			stmt.setInt(1, channelId);
			ResultSet results = stmt.executeQuery();
	...
	public static ArrayList<Payment> getPaymentsRefunded (Connection conn, int channelId) throws SQLException {
		...
		try {
			ArrayList<Payment> list = new ArrayList<Payment>();
			stmt = conn.prepareStatement("SELECT * FROM payments WHERE ( ( (channel_id_sender=? AND include_in_sender_channel=0 AND phase=6) OR " + "" +
					"(channel_id_receiver=? AND include_in_receiver_channel=0 AND phase=6) ) ) ORDER BY id DESC");
			stmt.setInt(1, channelId);
			stmt.setInt(2, channelId);
			ResultSet results = stmt.executeQuery();
	...
	public static ArrayList<Payment> getPaymentsSettled (Connection conn, int channelId) throws SQLException {
		...
		try {
			ArrayList<Payment> list = new ArrayList<Payment>();
			stmt = conn.prepareStatement("SELECT * FROM payments WHERE ( ( (channel_id_sender=? AND include_in_sender_channel=0 AND phase=1) OR " + "" +
					"(channel_id_receiver=? AND include_in_receiver_channel=0 AND phase=2) ) ) ORDER BY id DESC");
			stmt.setInt(1, channelId);
			stmt.setInt(2, channelId);
			ResultSet results = stmt.executeQuery();
	...
	public static void updateChannel (Connection conn, Channel channel) throws SQLException {
		...
		try {
			...
			// refund_tx_client=?, channel_tx_server=?, channel_tx_revoke_server=?, channel_tx_client=?, channel_tx_revoke_client=?, has_open_payments=?,
			// establish_phase=?, is_ready=?, key_chain_depth=?, key_chain_child=?, channel_tx_server_temp=?, channel_tx_client_temp=?,
			// channel_tx_revoke_client_temp=?, channel_tx_revoke_server_temp=?, payment_phase=?, master_chain_depth=?, opening_tx_hash=?,
			// change_address_server=?, pub_key_server=?, pub_key_client=? WHERE id=?");
			stmt = conn.prepareStatement("UPDATE channels SET master_priv_key_client=?, master_priv_key_server=?, initial_amount_server=?, " +
					"initial_amount_client=?, amount_server=?, amount_client=?, timestamp_open=?, timestamp_close=?, opening_tx=?, refund_tx_server=?, " +
					"refund_tx_client=?, channel_tx_server=?, channel_tx_revoke_server=?, channel_tx_client=?, channel_tx_revoke_client=?, " +
					"has_open_payments=?, establish_phase=?, is_ready=?, channel_tx_server_temp=?, channel_tx_client_temp=?, " +
					"channel_tx_revoke_client_temp=?," +
					" " +
					"channel_tx_revoke_server_temp=?, payment_phase=?, master_chain_depth=?, opening_tx_hash=?, change_address_server=?, pub_key_server=?, " +
					"pub_key_client=? WHERE id=?");
	...
	public static void updatePayment (Connection conn, Payment payment) throws SQLException {
		...
		try {
			payment.updateTransactionsToDatabase(conn);
			stmt = conn.prepareStatement("UPDATE payments SET channel_id_sender=?, channel_id_receiver=?, amount=?, phase=?, secret=?, " +
					"settlement_tx_sender=?," +
					" refund_tx_sender=?, settlement_tx_receiver=?, refund_tx_receiver=?, timestamp_created=?, timestamp_settled=?, " +
					"include_in_sender_channel=?, include_in_receiver_channel=?, add_tx_sender=?, add_tx_receiver=?, settlement_tx_sender_temp=?, " +
					"refund_tx_sender_temp=?, add_tx_sender_temp=?, settlement_tx_receiver_temp=?, refund_tx_receiver_temp=?, add_tx_receiver_temp=?, " +
