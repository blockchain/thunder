...
public class KeyChain {
	...
	public void start () throws Exception {
		...
		// bitcoinj is working a lot with the Google Guava libraries. The WalletAppKit extends the AbstractIdleService. Have a look at the introduction to
		// Guava services: https://code.google.com/p/guava-libraries/wiki/ServiceExplained
		kit.startAsync();
		kit.awaitRunning();

		//        kit.wallet().reset();

		// To observe wallet events (like coins received) we implement a EventListener class that extends the AbstractWalletEventListener bitcoinj then calls
		// the different functions from the EventListener class
		...
		for (TransactionOutput o : wallet.calculateAllSpendCandidates()) {
			...
			if (o.getParentTransaction().getConfidence().getDepthInBlocks() >= Constants.MIN_CONFIRMATION_TIME) {
				try {
					...
					output.setPrivateKey(new String(Base64.encode(wallet.findKeyFromPubHash(o.getAddressFromP2PKHScript(Constants.getNetwork()).getHash160())
							.getPrivKeyBytes())));
