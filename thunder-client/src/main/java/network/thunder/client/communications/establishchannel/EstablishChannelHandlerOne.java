...
//		System.out.println(transactionFromServer.getOutput(1).bitcoinSerialize().length);
		...
		if(! ( transactionFromServer.getOutput(0).getValue().value == (channel.getAmountClient() + channel.getAmountServer() ) ) )
			throw new Exception("Channel output does not contain the proper value. Got: " +transactionFromServer.getOutput(0).getValue().value + " Should be: "+ ( channel.getAmountClient() + channel.getAmountServer() ) );
