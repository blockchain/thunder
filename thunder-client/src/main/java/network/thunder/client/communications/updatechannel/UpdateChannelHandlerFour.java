...
public class UpdateChannelHandlerFour {
	...
	public void evaluate (UpdateChannelResponseFour m) throws Exception {
		...
		for (int i = 0; i < newPaymentsTotal.size(); ++i) {
			...
			if (payment.paymentToServer) {
				...
				ScriptTools.checkTransaction(additional, (i + 2), serverHash, payment.getAmount(), channel.getChangeAddressClient(), channel
						.getTimestampRefunds());
			...
			if (Tools.checkSignature(refund, 0, channelTransaction.getOutput(i + 2), channel.getServerKeyOnClient(), refund.getInput(0).getScriptSig()
					.getChunks().get(1).data)) {
				throw new Exception("Refund Server Signature is not correct");
			}
			if (Tools.checkSignature(settlement, 0, channelTransaction.getOutput(i + 2), channel.getServerKeyOnClient(), refund.getInput(0).getScriptSig()
					.getChunks().get(1).data)) {
		...
		ScriptTools.checkTransaction(revoke, 0, clientHash, channel.getChannelTxClientTemp().getOutput(0).getValue().value - Tools.getTransactionFees
				(Constants.SIZE_REVOKE_TX), channel.getChangeAddressClient(), channel.getTimestampRefunds());

		//		if(Tools.checkSignature(revoke, 0, channelTransaction.getOutput(0), channel.getClientKeyOnServer(), revoke.getInput(0).getScriptSig()
		// .getChunks().get(1).data))
	...
	public UpdateChannelRequestFour request () throws Exception {
		...
		revokeTransaction.addOutput(Coin.valueOf(channel.getChannelTxServerTemp().getOutput(1).getValue().value - Tools.getTransactionFees(Constants
				.SIZE_REVOKE_TX)), channel.getChangeAddressServerAsAddress());
