...
public class SendPaymentHandlerOne {
	...
	public void evaluateResponse (SendPaymentResponseOne m) throws Exception {
		...
		if (!receivedTransaction.getOutput(1).getAddressFromP2PKHScript(Constants.getNetwork()).toString().equals(channel.getChangeAddressServer())) {
			throw new Exception("Revoke Script to Server does not pay to correct address.. Should Be: " + channel.getChangeAddressServer() + " Is: " +
					receivedTransaction.getOutput(1).getAddressFromP2PKHScript(Constants.getNetwork()));
		}
	...
	public SendPaymentRequestOne request () throws Exception {
		...
		channelTransaction.addOutput(Coin.valueOf(channel.getAmountServer() + Tools.getTransactionFees(Constants.SIZE_REVOKE_TX)), ScriptTools.getRevokeScript
				(keyWrapper, channel, Constants.SERVERSIDE));
