	...
	public void updateTransactionsToDatabase (Connection conn) throws SQLException {
		...
		if (paymentToServer) {
			...
			if (refundTxReceiverChanged) {
				refundTxReceiverID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(refundTxReceiver, channelIdReceiver,
						refundTxReceiverID));
			}
