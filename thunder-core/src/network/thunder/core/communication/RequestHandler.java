...
import network.thunder.core.communication.objects.*;
import network.thunder.core.database.MySQLConnection;
import network.thunder.core.database.objects.*;
import network.thunder.core.etc.Constants;
import network.thunder.core.etc.ScriptTools;
import network.thunder.core.etc.Tools;
import network.thunder.core.etc.TransactionStorage;
...
public class RequestHandler {
	...
	public Message handle (String target, Message message) throws IOException {
		...
		try {
			...
			if (message.type == Type.ESTABLISH_CHANNEL_ONE_REQUEST) {
				...
				channelTransaction = MySQLConnection.getOutAndInputsForChannel(conn, channel.getId(), m.totalAmount - m.clientAmount, channelTransaction,
						wallet.getChangeAddress(), false, false);
				 ...
				 */
				System.out.println(transactionFromClient);
				...
				/**
				 * Reset the temp-fields in the payment database, since we create a new temp-tx here.
				 */
				...
				if (channel.getChannelTxClientID() != 0) {
					if (m.amount + 2 * Tools.getTransactionFees(Constants.SIZE_OF_SETTLEMENT_TX) > channel.getChannelTxClient().getOutput(0).getValue()
							.value) {
				...
				if (channel.getChannelTxServerID() != 0) {
					...
					for (int i = 0; i < paymentList.size(); i++) {
						...
						if (!ScriptTools.checkPaymentScript(client, channel, keyWrapper, payment.getSecretHash(), Constants.SERVERSIDE, payment
								.paymentToServer)) {
				...
				if (receivedTransaction.getOutput(1).getValue().value < (channel.getAmountServer() + Tools.getTransactionFees(Constants.SIZE_REVOKE_TX))) {
					throw new Exception("Server change is not correct. Is: " + receivedTransaction.getOutput(1).getValue().value + " Should be: " + (channel
							.getAmountServer() + Tools.getTransactionFees(Constants.SIZE_REVOKE_TX)));
				}
				...
				if (!receivedTransaction.getOutput(0).getAddressFromP2PKHScript(Constants.getNetwork()).toString().equals(channel.getChangeAddressClient())) {
					throw new Exception("Revoke Script to Client does not pay to correct address.. Is: " + receivedTransaction.getOutput(0)
							.getAddressFromP2PKHScript(Constants.getNetwork()).toString() + " Should Be: " + channel.getChangeAddressClient());
				}
				...
				MySQLConnection.updateChannel(conn, channel);
				System.out.println("New Payment Added: " + newPayment.getSecretHash());
				MySQLConnection.addPayment(conn, newPayment);

				/**
				 * TODO: This isn't really optimized, when all we want is to have all
				 *      payments currently in the channel to have temp-channel=true..
				 */
				...
				for (int i = 0; i < paymentList.size(); ++i) {
					...
					if (Tools.checkSignature(refund, 0, receivedTransaction.getOutput(i + 2), channel.getClientKeyOnServer(), refund.getInput(0).getScriptSig
							().getChunks().get(1).data)) {
						throw new Exception("Refund Client Signature is not correct");
					}
					if (Tools.checkSignature(settlement, 0, receivedTransaction.getOutput(i + 2), channel.getClientKeyOnServer(), refund.getInput(0)
				...
				revokeTransaction.addOutput(channel.getChannelTxClientTemp().getOutput(0).getValue().subtract(Coin.valueOf(Tools.getTransactionFees(Constants
						.SIZE_REVOKE_TX))), channel.getChangeAddressClientAsAddress());
				 ...
				 */
				ECDSASignature serverSig = Tools.getSignature(revokeTransaction, 0, channel.getChannelTxServerTemp().getOutput(1), channel
						.getServerKeyOnServer());
				...
				for (Payment p : paymentList) {
					...
					p.setIncludedInChannelTemp(false);
				 ...
				 *
				 * Request: 	New key to be included in the clientside channel transaction.
				 ...
				 *
				 * Response: 	New key to be included in the serverside channel transaction.
				 * 				Array with secretHashes of payments, that will be removed.
				 * 				Array with secrets of payments that are currently included in the channel
					...
					for (Payment p : paymentList) {
						if (!p.paymentToServer) {
							if (p.getSecretHash().equals(secret.secretHash)) {
								System.out.println("Secret Exposed: " + p.getSecretHash());
								/**
								...
								MySQLConnection.updatePayment(conn, secret.secretHash, secret.secret);
				 ...
				 *
				 * We can add an unlimited amount of payments here, since we are no longer limited by the keys..
				 */
				ArrayList<Payment> newPayments = MySQLConnection.getPaymentsIncludedInChannelWithPaymentsNotAddedYet(conn, channel.getId(), 99999);
				...
				Transaction t = channel.getOpeningTx();
				channelTransaction.addInput(channel.getOpeningTx().getOutput(0));
				...
				if (receivedTransaction.getOutput(1).getValue().value < (channelTransaction.getOutput(1).getValue().value + Tools.getTransactionFees(Constants
						.SIZE_REVOKE_TX))) {
				...
				for (int i = 0; i < paymentList.size(); ++i) {
					...
					if (payment.paymentToServer) {
						ScriptTools.checkTransaction(refund, (i + 2), channelHash, payment.getAmount(), channel.getChangeAddressClient(), channel
								.getTimestampRefunds());
				 ...
				 */
				Transaction revoke = new Transaction(Constants.getNetwork(), Tools.stringToByte(m.revokeTransaction));
				ScriptTools.checkTransaction(revoke, 1, channelHash, channel.getChannelTxClientTemp().getOutput(1).getValue().value, channel
