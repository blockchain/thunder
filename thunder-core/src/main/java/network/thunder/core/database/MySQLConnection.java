...
import network.thunder.core.communication.objects.EstablishChannelRequestOne;
...
public class MySQLConnection {
	...
	public static void addKey (Connection conn, ArrayList<Key> keyList, int channelId, boolean ourKey) throws SQLException {
		...
		/* Workaround to make sure we got NULL and not "" in the DB	 */
		if (!ourKey) {
			for (Key key : keyList) {
				key.privateKey = null;
			}
	...
	public static int addPayment (Connection conn, Payment payment) throws SQLException {
		PreparedStatement stmt = null;
		try {
			stmt = conn.prepareStatement("INSERT INTO payments values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "", Statement
					.RETURN_GENERATED_KEYS);
	...
	public static void checkKeysFromOtherSide (Connection conn, Channel channel, ArrayList<Key> newKeys) throws Exception {
		//TODO: Change to hash-based keys, with a new way of calculating the hierarchy..
		if (newKeys == null) {
		...
		try {
			...
			ArrayList<String> currentKeys = ScriptTools.getPubKeysOfChannel(channel, true, true);
	...
	public static ArrayList<Key> createKeys (Connection conn, Channel channel, int amount) throws SQLException {
		//TODO: Switch over to hash-based keys..
		Statement stmt = null;
		ResultSet set = null;
		try {
	...
	public static Channel createNewChannelOnRequest (Connection conn, EstablishChannelRequestOne message, String changeAddressServer, String
			changeAddressClient, int nodeId) throws SQLException {
		...
		try {
			...
			int i = 1;
			stmt.setString(i++, null);
			stmt.setInt(i++, nodeId);

			stmt.setString(i++, message.getPubKey());
			stmt.setString(i++, Tools.byteToString(key.getPubKey()));
			stmt.setString(i++, changeAddressServer);
			stmt.setString(i++, changeAddressClient);
			stmt.setString(i++, masterKey);
			stmt.setString(i++, null);
			stmt.setLong(i++, 0);
			stmt.setLong(i++, 0);
			stmt.setLong(i++, 0);
			stmt.setLong(i++, 0);
			stmt.setInt(i++, Tools.currentTime());
			stmt.setInt(i++, 0);
			stmt.setInt(i++, 9999999); //TODO: Hack to ensure we start at a high depth..
			stmt.setInt(i++, 0);
			stmt.setInt(i++, 0);
			stmt.setString(i++, null);
			stmt.setString(i++, null);
			...
			Channel channel = MySQLConnection.getChannel(conn, message.getPubKey());
	 ...
	 * @throws SQLException the SQL exception
	 */
	public static Channel createNewChannelForRequesting (Connection conn, String changeAddressClient, int nodeId) throws SQLException {
		PreparedStatement stmt = null;
		try {

			stmt = conn.prepareStatement("INSERT INTO channels (node_id, pub_key_client, change_address_client, master_priv_key_client, phase, " +
					"is_ready)" +
					" " +
					"values (?, ?, ?, ?, ?, ?)");
	...
	public static int getCurrentPaymentsAmount (Connection conn, Channel channel) throws SQLException {
		...
		ResultSet set = null;
		try {
			stmt = conn.prepareStatement("SELECT count(*) FROM payments WHERE (channel_id_sender=? AND include_in_sender_channel=1) OR " +
					"(channel_id_receiver=?" +
					" " +
	...
	public static Connection getInstance () throws SQLException {
		//TODO: Move this method away into the actual implementation, to keep deployment separated..
		// if (conn == null)
		// new MySQLConnection();
		// return conn;
		try {
	 ...
	 * @throws Exception the exception
	 */
	public static Key getKeyCurrentlyUsed (Connection conn, Channel channel) throws Exception {
		PreparedStatement stmt = null;
		try {
			String sql = "SELECT priv_key FROM storedkeys WHERE (channel_id=? AND current_channel=1)";
			...
			if (!set.first()) {
				System.out.println("getKeyCurrentlyUsed return null..: " + stmt.toString());
				set.close();
				return null;
			}
			Key key = new Key(set);
			...
			if (set.next()) {
				set.close();
				throw new RuntimeException("There should only be one key in use at a time..");
			}
	...
	public static ArrayList<Key> getKeysOfOtherSideToBeExposed (Connection conn, Channel channel) throws Exception {
		...
		try {
			...
			while (!set.isAfterLast()) {
				...
				if (expose) {
					keysToBeExposed.add(new Key(set));
				}
	...
	public static ArrayList<Key> getKeysOfUsToBeExposed (Connection conn, Channel channel, boolean exposeKeys) throws Exception {
		...
		try {
			...
			int lowestDepthToExpose = 9999999;
	...
	public static BiMap<Integer, String> getOpeningHashes (Connection conn, boolean ready) throws SQLException {
		...
		try {
			...
			String query = "SELECT opening_hash_client, id FROM channels WHERE is_ready=?";
	 ...
	 */
	public static Transaction createOpeningTransaction (Connection conn, Channel channel, Wallet wallet) throws SQLException, AddressFormatException {
		PreparedStatement stmt = null;
		ResultSet set = null;
		try {

			HashMap<String, Output> list = new HashMap<>();

			for (TransactionOutput out : wallet.calculateAllSpendCandidates()) {
				Output o = new Output(out);
				list.put(o.getHash()+o.getVout(), o);
			}

			stmt = conn.prepareStatement("SELECT * FROM outputs WHERE timestamp_locked>?");
			...
			while (!set.isAfterLast()) {
				list.remove(set.getString("transaction_hash")+set.getInt("vout"));
			}
			 ...
			 *
			 * We always have to pay the fees for our opening tx.
			 */
			long value = (channel.getInitialAmountServer() + Tools.getTransactionFees(1, 1));
			...
			for (Output o : list) {
				if (o.getValue() >= value && transaction == null) {
					 ...
					 */
					transaction = new Transaction(Constants.getNetwork());
					/*
	...
	public static ArrayList<Payment> getPaymentsOpen (Connection conn, int channelId) throws SQLException {
		...
		try {
			ArrayList<Payment> list = new ArrayList<Payment>();
			stmt = conn.prepareStatement("SELECT * FROM payments WHERE ( channel_id_receiver=? AND include_in_receiver_channel=0 AND phase_receiver=0 ) " +
					"ORDER" +
					" " +
					"" + "BY id DESC");
			stmt.setInt(1, channelId);
